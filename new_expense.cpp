...
void new_expense::set_category() {
    category_data = file_operations::readfile("./data/categories/expense.txt");
    ...
    for (int i = 0; i < vector_size; i++) {
        std::cout << i << " - " << category_data[i] << "\n";
    }
    std::cout << vector_size << " - Add a new category" << "\n";
    std::cout << vector_size + 1 << " - Remove a category" << "\n";
    ...
    if (user_choice_i == vector_size) {
        ...
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); // Clear the cin buffer before call to getline()
        ...
        file_operations::writefile("./data/categories/expense.txt", category_data);
        new_expense::set_category(); // This is needed to get the cin buffer to work correctly with the set_notes() function.
        ...
        if (category_data.size() > 0) { // Check if there is no data to be removed in the 'category_data' vector
            file_operations::delete_category("./data/categories/expense.txt", category_data);
            new_expense::set_category();
            return;
        } else {
...
void new_expense::set_sum() {
    std::cout << "Please input the expense amount with '.' as a decimal separator: ";
    std::cin >> sum;
